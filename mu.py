...
class FullParser:
    ...
    def mech_action(self, drugWebsite, current_entry, indexNumber):
        bob=[]
        baselines=[]
        pwl = enchant.request_pwl_dict("mechanisms.txt")
        start=0;
    ...
    def UltimateAnalytics(self,current_entry, drugWebsite):
        ...
        for q in range(0, len(totalChecks)):
           # print(current_entry)
            ...
            finalDrugName=self.adderCheck(drugWebsite, current_entry, totalChecks[q])
            mechanis_info=self.mech_action( drugWebsite, current_entry, totalChecks[q])
            #print(mechanis_info);
            if presetValue==[]:
            ...
            known=self.TruthCheck(pre,phasing)
            #fullList=[True,]
            joe=[self.company_name,finalDrugName,pre,phasing,mechanis_info]
            bob.append(joe)
            truthful.append(known)
        bobby=self.FullReference(truthful,bob)
    ...
    def FullReference(self,current_entry, listValues):
        ...
        for q in range(0, len(current_entry)):
            ...
            if indi3==1 and indi2==1:
                ...
                newV=[temp[0],temp[1],temp[2],temp2[3],holder]
...
# Any testing code goes here
    def adderCheck(self, drugWebsite, current_entry, indexNumber):
        
        
        if '+' in drugWebsite[indexNumber+1]:
            if self.checking_official(drugWebsite[indexNumber+2])!='null':
                
                temp=drugWebsite[indexNumber]
                temp2=drugWebsite[indexNumber+2]
                temp3='+'
                ...
                return drugWebsite[indexNumber]
        elif '+' in drugWebsite[indexNumber]:
            if self.checking_official(drugWebsite[indexNumber+2])!='null':
                if '+' in drugWebsite[indexNumber]:
                     temp=drugWebsite[indexNumber]
                temp2=drugWebsite[indexNumber+2]
                temp3='+'
                intermediate=temp+temp3+temp2
